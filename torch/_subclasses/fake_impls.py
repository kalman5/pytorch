...
def fast_detach(fake_mode, x, include_real=False):
    ...
    if include_real:
        return FakeTensor(fake_mode, out, x.device, real_tensor=x.real_tensor, strong_fake_mode=True)
    return FakeTensor(fake_mode, out, x.device, strong_fake_mode=True)
